...
const oldMarqueeStart = text.indexOf('function CompaniesMarquee() {');
const oldMarqueeEnd = text.indexOf('function AboutSection() {');
if (oldMarqueeStart !== -1 && oldMarqueeEnd !== -1) {
  let before = text.substring(0, oldMarqueeStart);
  let after = text.substring(oldMarqueeEnd);
  
  const newTrustedSection = `function TrustedBySection() {
...
`;
  ...
  text = before + newTrustedSection + after;
  
  // Need to replace <CompaniesMarquee /> inside AboutSection return statement
  text = text.replace(/<CompaniesMarquee \/>/g, '<TrustedBySection />');
